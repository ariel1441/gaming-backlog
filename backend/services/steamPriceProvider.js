...
export function selectSteamPriceOffer(item, appId) {
  ...
  if (Number(item.type || 0) !== 0) throw invalid('This listing is not a standalone game; DLC and other content are not priced.', 'steam_price_unsupported_type');
  ...
  const standard = candidate => candidate?.packageid && !candidate.bundleid && Number(candidate.included_game_count) === 1 &&
...
export function normalizeSteamPrice(item, appId, packagePayload = null, observedAt = new Date().toISOString()) {
  ...
  if (response?.success !== true || !Array.isArray(data?.apps) || data.apps.length !== 1 || String(data.apps[0]?.id) !== String(appId) ||
      normalizedName(data.name) !== normalizedName(option.purchase_option_name) ||
      (data.apps[0].name && normalizedName(data.apps[0].name) !== normalizedName(item.name))) throw invalid('Steam package contents do not confirm the selected game.', 'steam_price_package_mismatch');
  const p = data.price;
  ...
  return { ...base, currency: 'ILS', offerId: `package:${option.packageid}`, offerName: option.purchase_option_name,
    ...
    evidence: { ...base.evidence, packageId: option.packageid, packageApps: [String(appId)], currencyProvenance: 'package_price',
...
export async function fetchSteamPrices(appIds, { beforeRequest = async () => {} } = {}) {
  ...
  for (const appId of appIds) {
    try {
      ...
      results.push({ appId, observation: normalizeSteamPrice(item, appId, details) });
    } catch (error) {
