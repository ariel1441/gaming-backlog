          ...
          <div className="p-5 md:p-6">
            <div className="min-w-0">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <ProfileStat icon={CheckCircle2} label="Completed" value={snapshot.stats.completed} />
                <ProfileStat icon={Clock3} label="Hours" value={`${Math.round(snapshot.stats.hours)}h`} />
                <ProfileStat
                  icon={Star}
                  label="Avg score"
                  value={
                    snapshot.stats.avgScore == null
                      ? "N/A"
                      : `${snapshot.stats.avgScore.toFixed(1)}/10`
                  }
                />
              </div>
            </div>
  ...
  return (
    <section className="rounded-2xl border border-surface-border bg-surface-card p-4">
      <div className="flex items-end justify-between gap-3">
        ...
        <span className="text-xs text-content-muted">Top {placeholderCount}</span>
      </div>
