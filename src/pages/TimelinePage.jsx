...
function NoCoverBackdrop({ title }) {
  return (
    <div className="absolute inset-0 overflow-hidden bg-surface-card">
      <div className="absolute inset-0 bg-[linear-gradient(135deg,rgba(255,255,255,0.08)_0,rgba(255,255,255,0.08)_1px,transparent_1px,transparent_18px)] opacity-20" />
...
function PosterMedia({ game }) {
  ...
  return (
    <div className="relative h-48 w-full shrink-0 overflow-hidden rounded-2xl border border-surface-border bg-surface-elevated shadow-xl shadow-black/25 sm:h-64 sm:w-[22rem] lg:h-72 lg:w-[30rem]">
      {src ? (
        <>
          <img
            src={src}
...
function ViewModeToggle({ viewMode, setViewMode }) {
  return (
    <div className="inline-flex rounded-xl border border-surface-border bg-surface-elevated/70 p-1">
        ...
        <button
          ...
          className={[
            ...
            viewMode === option.value
              ? "bg-action-primary text-white shadow-sm shadow-primary/15"
              : "text-content-secondary hover:bg-surface-border hover:text-content-primary",
          ].join(" ")}
        >
          {option.label}
        </button>
...
function TimelineHeader({ summary, viewMode, setViewMode }) {
  ...
  return (
    <header className="border-b border-surface-border pb-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-semibold text-content-primary">Timeline</h1>
            <p className="text-sm text-content-secondary">
              Started and finished dates from your backlog, grouped by month.
            </p>
          </div>
        </div>
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        ...
        <StatPill icon={CheckCircle2} label="Finished" value={summary.finished} />
...
function TimelineFilters({
  ...
  return (
    <section className="rounded-2xl border border-surface-border bg-surface-card/75 p-4">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(16rem,1fr)_auto] lg:items-center">
  ...
  return (
    <div className="relative flex min-h-full items-center justify-center">
      ...
      <div className="relative z-10 flex flex-col items-center gap-2">
        <div className="rounded-2xl border border-surface-border bg-surface-bg/95 px-2 py-2 text-center shadow-lg shadow-black/25 sm:px-2.5">
        ...
        <div
          className={[
            "flex h-10 w-10 items-center justify-center rounded-full border bg-surface-bg shadow-lg shadow-black/30 sm:h-12 sm:w-12",
            meta.border,
          ].join(" ")}
        >
          <Icon className={["h-5 w-5 sm:h-6 sm:w-6", meta.text].join(" ")} aria-hidden="true" />
        </div>
      </div>
    </div>
  );
...
function CurrentStatusBadge({ status }) {
  return (
    <div className="inline-flex min-w-0 max-w-full items-center gap-2 rounded-full border border-white/15 bg-black/30 px-2 py-1 backdrop-blur-md">
...
function PosterEvent({ event, onOpen }) {
  const meta = eventCopy[event.type];
  const Icon = meta.icon;

  return (
    <article className="grid grid-cols-[4.25rem_minmax(0,1fr)] gap-3 sm:grid-cols-[6.5rem_minmax(0,1fr)] sm:gap-4">
      ...
      >
        <div className="flex flex-col gap-4 sm:gap-6 lg:flex-row">
          ...
          <div className="flex min-w-0 flex-1 flex-col justify-center py-1">
            <div className={["flex items-center gap-2 text-base font-semibold", meta.text].join(" ")}>
...
function BackdropEvent({ event, onOpen }) {
  const meta = eventCopy[event.type];
  ...
  return (
    <article className="grid grid-cols-[4.25rem_minmax(0,1fr)] gap-3 sm:grid-cols-[6.5rem_minmax(0,1fr)] sm:gap-4">
      ...
      >
        ...
        <div className={["absolute inset-y-0 left-0 w-1.5", meta.accent].join(" ")} />
        ...
        <div className="relative flex min-h-48 flex-col justify-between gap-8 p-5 sm:min-h-60 sm:gap-12 sm:p-7 lg:min-h-64">
          ...
          <div className="max-w-3xl pb-2 sm:pb-4">
            <h2 className="line-clamp-2 pb-2 text-2xl font-semibold leading-[1.18] text-white drop-shadow-lg sm:text-4xl lg:text-5xl">
              {event.title}
            </h2>
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <CurrentStatusBadge status={event.game?.status} />
...
function TimelineGroup({ group, viewMode, onOpen }) {
  const EventComponent = timelineViews[viewMode]?.component || BackdropEvent;

  return (
    <section className="space-y-4">
      <div className="grid grid-cols-[4.25rem_minmax(0,1fr)] items-center gap-3 sm:grid-cols-[6.5rem_minmax(0,1fr)] sm:gap-4">
  ...
  return (
    <main className="min-h-screen bg-surface-bg px-4 py-6 text-content-primary sm:px-6">
      <div className="mx-auto max-w-7xl space-y-5">
        <div className="h-24 animate-pulse rounded-2xl border border-surface-border bg-surface-card" />
        <div className="h-16 animate-pulse rounded-2xl border border-surface-border bg-surface-card" />
        {Array.from({ length: 3 }).map((_, index) => (
          <div
            key={index}
            className="ml-auto h-52 max-w-5xl animate-pulse rounded-2xl border border-surface-border bg-surface-card"
          />
        ))}
      </div>
    </main>
...
export default function TimelinePage() {
  ...
  const summary = useMemo(() => summarizeTimeline(games, events), [games, events]);
  const years = useMemo(
    ...
    <main className="min-h-screen bg-surface-bg px-4 py-6 text-content-primary sm:px-6">
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        {groups.length ? (
          ...
          <EmptyState
            icon={timelineViews[viewMode]?.component === BackdropEvent ? Clock3 : PlayCircle}
            title={
              events.length
