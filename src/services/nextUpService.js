import { api } from "./apiClient";

export function getNextUp(opts = {}) {
  return api.get("/api/next-up", opts);
}

export function addToNextUp(gameId, role = "main", opts = {}) {
  return api.post(`/api/next-up/${gameId}`, { role }, opts);
}

export function removeFromNextUp(gameId, opts = {}) {
  return api.del(`/api/next-up/${gameId}`, opts);
}

export function reorderNextUp(gameIds, role, opts = {}) {
  return api.put("/api/next-up/reorder", { gameIds, role }, opts);
}

export function startPlaying(gameId, opts = {}) {
  return api.post(`/api/next-up/${gameId}/start`, undefined, opts);
}

export function assignPlayFocus(role, gameId, opts = {}) {
  return api.put(`/api/next-up/focus/${role}`, { gameId }, opts);
}

export function removePlayFocus(gameId, opts = {}) {
  return api.del(`/api/next-up/focus/${gameId}`, opts);
}
