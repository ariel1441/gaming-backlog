...
may be superseded; consult the current NEXT_TASKS.md and live code/Git first.
...
Review the local [fix phase record](daily_sync__wishlist_and_loaded/release-review-fixes.md),
then prepare coherent commits and exact-candidate CI when authorized. Nothing has
been committed or pushed in this fix phase. Migration 035 still needs normal
environment application during the authorized release process. The September 8
handoff below is retained as implementation context, superseded by this checkpoint.

Current Steam handoff: visually review the approved notification iteration and
independently review the uncommitted C.5 implementation
above `21dfd7e` on `fix/steam-candidate-account-isolation`. Read the
[C.5 implementation record](daily_sync__wishlist_and_loaded/gaming-backlog-steam-phase-c5-implementation-record.md).
The [notification implementation](daily_sync__wishlist_and_loaded/notification-panel-design.md)
adds the toolbar bell, shared status choices, acquisition/playing quick actions,
snapshot-based Hide all, optional local Wishlist retirement, and Settings diagnostics.
Migration 034 is applied to localhost; focused Node and desktop/mobile checks
passed. Full CI, any commit/release and production scheduling remain separate
pending tasks. No offer expansion or live coverage probe was performed in C.5.
Loaded stays Phase D, Fanatical E and remaining deal/notification polish F.
