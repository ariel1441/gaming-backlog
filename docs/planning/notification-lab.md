...
It is visible in **Settings → Integrations** only in a Vite development build, and its API returns `404` outside a localhost non-production request. It never exists in a production deployment.

## Use

Sign in to a normal local account (not a guest demo session), then choose a scenario and select **Create demo**. Reload the app once it reports success, then open the normal notification bell.
...
- `sale-ended` — grouped sale-ended and price-increase facts, shown under Other updates for anti-spam review
